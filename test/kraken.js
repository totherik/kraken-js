...
test('uncaughtException', function (t) {

    var exit, listeners;

    function patch(fn) {
        // Ensure there aren't any listeners to our default behavior
        // is triggered.
        listeners = process.listeners('uncaughtException');
        process.removeAllListeners('uncaughtException');

        // Intercept `process.exit` to ensure it gets called
        exit = process.exit;
    ...
    function unpatch() {
        // Revert process.ext to original and re-attach listeners
        process.exit = exit;
        listeners.forEach(process.on.bind(process, 'uncaughtException'));
    }

    t.test('should provide a default handler', function (t) {
        var server;
        ...
        kraken.create().listen(8000, function (err, srvr) {
            t.error(err);
            server = srvr;
